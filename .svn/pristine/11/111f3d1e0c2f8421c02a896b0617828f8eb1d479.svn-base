<template>
   ...
   <div class="Prescription-background">
      ...
      <ul >
         <li v-for="list in lists"  @click="detail(list.recipe.recipeId,list.order.module)">
            <div>
              <div>{{list.recipe.createDate == null||list.recipe.createDate == ''?"暂无": list.recipe.createDate.substring(0,10)}}</div>
              <div>
                 <span id="NotPay" v-if="list.type == 'no'|| list.order.status == 0 ">待付款</span>
                 <span id="alreadyPay" v-else-if="(list.order.status == 1 && list.order.module =='354'||'355') == true">已确认</span>
                 <span id="alreadyPay" v-else>已付款</span>
              </div>
              <div>
...
export default {
  ...
  methods:{
    query(){        //请求数据.
        Indicator.open('加载中...');
        this.axios.get( this.url_s+'/recipeController/findRecipe',{
            params:{
                 page: this.nums,
