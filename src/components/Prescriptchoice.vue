    ...
    <div class="perscript_one">
        ...
        <div>
            <div>
                ...
                <div>
                    ...
                    <div><span @click="submit(lists.order.orderId)">{{lists.type == 'no' || lists.order.status == '0'?buttxt="到店支付":"药店详情"}}</span></div>
                </div>
            </div>
        </div>
        <div class="bg" v-if="bg">
            <div :class="{marg :check==false}">
                ...
                <div v-if="lists.type == 'no' || lists.order.status == '0'">您的药品订单将推送到药房，请点击确认按钮后尽快到药房付款取药！</div>
                ...
                <div >
                    <div @click="Isclick" v-if="lists.type == 'no' || lists.order.status == '0' ">如果您是住院病人请填写以下内容 ∨</div>
                    <div v-if="check">
                ...
                <div @click="getpay(lists.order.orderId,lists.yuyue.hospitalId)" v-if="lists.type == 'no' || lists.order.status == '0'">确认</div>
            </div>
        </div>
    </div>
   </div>
  ...
  methods: {
    ...
    getMedical(hospitalId) {
      this.axios        //正式环境接口this.$store.state.schlwyyUrl   //测试地址 this.hlwyyUrl
        .get( this.$store.state.schlwyyUrl + "/recipe-management/api/institutions", {
          params: {
            "page-index": "1",
            "page-size": "5",
    ...
    submit(orderId){
        this.bg = true
        /* let medicineName = "";
        for(var key in this.lists.medicines){
            medicineName +=this.lists.medicines[key].medicine.medicinePublic.medicineName + ",";
        }
        console.log(medicineName)
        this.axios.get( this.url_s +"/recipeController/getMedicineMyself",{
            params:{
                recipeId: this.$route.query.recipeId,
                orderId:orderId,
                ...
                pharmacyName:  this.medicalAd.name,
                pharmacyAddree: this.medicalAd.address,
                pharmacyTel:this.medicalAd.telephone
                }
        }).then(res=>{
            if (res.data == "success") {
                this.success = false;
